...
# Command line setup ----
# If running on from the shell, load other Maaslin2 modules
...
options <-
  optparse::add_option(
    ...
    help = paste0(
      "The minimum percent of samples for which",
...
options <-
  optparse::add_option(
    ...
    help = paste(
      "The maximum number of scatterplots for signficant",
  ...
  function(input_data,
    ...
    for (limited_transform in hash::keys(valid_choice_transform_norm)) {
      if (transform == limited_transform) {
        if (!normalization %in%
          ...
          option_not_valid_error(
            paste0(
              "This transform can only be used",
              " with a subset of normalizations. ",
              "Please select from the following list"
            ),
            toString(
    ...
    if (transform != "NONE") {
      if (!analysis_method %in% valid_choice_method_transform) {
        option_not_valid_error(
          paste0(
            "The transform selected can only be used",
            " with some methods. ",
            "Please select from the following list"
          ),
    ...
    for (i in fixed_effects) {
      ...
      if (is.factor(metadata[, i]) && !(i %in% split_reference)) {
        logging::loginfo(paste("Factor detected for categorial metadata '",
          i, "'. Provide a reference argument or manually set factor ordering to change reference level.",
          sep = ""
        ))
        next
      }
    ...
    # Write extracted random effects to file ----
    if (!is.null(random_effects)) {
